...
from modeling.qa.vis.modeling_mistral_dual_view import *
...
def evaluate_accuracy_test(eval_set, model, split, epoch_id):
    n_samples, n_correct = 0, 0
    model.eval()
    with torch.cuda.amp.autocast():
        with torch.no_grad():
            for qids, labels, *input_data in tqdm(eval_set):
                _, logits, _, emb_tuple= model(*input_data, split=split)
                n_correct += (logits.argmax(1) == labels).sum().item()
                n_samples += labels.size(0)
    
    return n_correct / n_samples

def evaluate_accuracy(eval_set, model, split, epoch_id):
    n_samples, n_correct = 0, 0
    model.eval()
    emb_list_text = []
    emb_list_graph = []
    with torch.cuda.amp.autocast():
        with torch.no_grad():
            for qids, labels, *input_data in tqdm(eval_set):
                ...
                n_correct += (logits.argmax(1) == labels).sum().item()
                n_samples += labels.size(0)
                emb_sent_text, emb_sent_graph = emb_tuple
                emb_list_text.append(emb_sent_text)
                emb_list_graph.append(emb_sent_graph)

    emb_list_text = torch.cat(emb_list_text, 0)
    emb_list_graph = torch.cat(emb_list_graph, 0)

    assert emb_list_text.size(0) == emb_list_graph.size(0)

    # save embeddings data/csqa/vis
    torch.save((emb_list_text, emb_list_graph), f'data/csqa/vis/{split}_emb_{epoch_id}.pt')
...
def main():
    ...
    parser.add_argument('--graph_text_embs_path', type=str, default=None)
    parser.add_argument('--graph_text_embs_path_dev', type=str, default=None)
    parser.add_argument('--graph_text_embs_path_test', type=str, default=None)
...
def train(args):
    ...
    if True:
        ...
        dataset = LM_QAGNN_DataLoader(args, args.train_statements, args.train_adj,
                                               ...
                                               graph_text_embs_path=args.graph_text_embs_path,
                                               graph_text_embs_path_dev=args.graph_text_embs_path_dev, 
                                               graph_text_embs_path_test=args.graph_text_embs_path_test)
    ...
    grouped_parameters = [
        ...
        {'params': [p for n, p in model.fc.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay, 'lr': args.decoder_lr},
        {'params': [p for n, p in model.fc.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': args.decoder_lr},
    ...
    # try:
        ...
        for epoch_id in range(args.n_epochs):
            ...
            for qids, labels, *input_data in tqdm(dataset.train()):
                ...
                if args.fp16:
                    with torch.cuda.amp.autocast():
                        logits_text, logits_graph, contrastive_loss, emb_tuple = model(*input_data, layer_id=args.encoder_layer)
                        loss = (compute_loss(logits_text, labels) + compute_loss(logits_graph, labels)) / 2
                        loss += 0.05 * contrastive_loss
                        mse_loss = nn.MSELoss()
                        loss += 0.01 * mse_loss(logits_text, logits_graph)
                ...
                emb_sent_text, emb_sent_graph = emb_tuple
                emb_list_text_list.append(emb_sent_text)
                emb_list_graph_list.append(emb_sent_graph)
            ...
            emb_list_text = torch.cat(emb_list_text_list, 0)
            emb_list_graph = torch.cat(emb_list_graph_list, 0)

            assert emb_list_text.size(0) == emb_list_graph.size(0)
            
            # save embeddings data/csqa/vis
            torch.save((emb_list_text, emb_list_graph), f'data/csqa/vis/train_emb_{epoch_id}.pt')

            model.eval()
            dev_acc = evaluate_accuracy(dataset.dev(), model, split='dev', epoch_id=epoch_id)
            save_test_preds = args.save_model
            # if not save_test_preds:
            test_acc = evaluate_accuracy(dataset.test(), model, split='test', epoch_id=epoch_id) if args.test_statements else 0.0
            # else:
