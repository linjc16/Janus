...
from modeling.retrieval.modeling_mistral_dual_view import *
from utils.optimization_utils import OPTIMIZER_CLASSES
from utils.parser_utils import *
import copy

from beir.retrieval.search.dense.util import dot_score, cos_sim
...
def evaluate_accuracy(eval_set, corpus_set, model, dataset_name, split='dev'):
    print('*' * 100)
    print("Evaluating on %s set" % split)
    print('*' * 100)
    ...
    with torch.cuda.amp.autocast():
        with torch.no_grad():
            query_embeddings = []
            query_ids = []
            for mix_ids, labels, *input_data in tqdm(eval_set):
                out_embeds = model.forward_inference(*input_data)
                query_embeddings.append(out_embeds)
                qids = [mix_id.split('_')[0] for mix_id in mix_ids]
                query_ids.extend(qids)

            query_embeddings = torch.cat(query_embeddings, dim=0)

            # deduplicate query_ids and delete the corresponding embeddings
            query_ids, unique_idx = np.unique(query_ids, return_index=True)
            query_embeddings = query_embeddings[unique_idx]

            result_heaps = {qid: [] for qid in query_ids}  # Keep only the top-k docs for each query
            results = {qid: {} for qid in query_ids}
            
            for cids, labels, *input_data in tqdm(corpus_set):
                sub_corpus_embeddings = model.forward_inference(*input_data)
                cos_scores = cos_sim(query_embeddings, sub_corpus_embeddings)
                cos_scores[torch.isnan(cos_scores)] = -1
                # Get top-k values
                cos_scores_top_k_values, cos_scores_top_k_idx = torch.topk(cos_scores, min(top_k+1, len(cos_scores[1])), dim=1, largest=True)
        ...
        dummy_retriever = EvaluateRetrieval()
        corpus, queries, qrels = GenericDataLoader(data_folder=RAW_FILE_PATH[dataset_name]).load(split=split)
        ndcg, _map, recall, precision = dummy_retriever.evaluate(qrels, results, k_values)

    print("Evaluation Results with k = %s" % k_values)
    print("NDCG@k: %s" % ndcg)
    print("MAP@k: %s" % _map)
...
def main():
    ...
    parser.add_argument('--graph_text_embs_query_path', type=str, default=None)
    parser.add_argument('--graph_text_embs_corpus_path', type=str, default=None)

    parser.add_argument('--all_corpus_statements_path', type=str, default=None)
    parser.add_argument('--all_corpus_adj_path', type=str, default=None)
    parser.add_argument('--lora', default=False, type=bool_flag, help='use lora')
...
def train(args):
    ...
    if True:
        ...
        dataset = LM_QAGNN_DataLoader_Retrieval(args, args.train_statements, args.train_adj,
                                               ...
                                               subsample=args.subsample, use_cache=args.use_cache, graph_text_embs_query_path=args.graph_text_embs_query_path,
                                               graph_text_embs_corpus_path=args.graph_text_embs_corpus_path,
                                               all_corpus_statements_path=args.all_corpus_statements_path,
                                               all_corpus_adj_path=args.all_corpus_adj_path)
        ...
        model.fc_text_view.to(device1)
        model.fc_graph_view.to(device1)
    ...
    grouped_parameters = [
        ...
        {'params': [p for n, p in model.fc_text_view.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay, 'lr': args.decoder_lr},
        {'params': [p for n, p in model.fc_text_view.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': args.decoder_lr},
        {'params': [p for n, p in model.fc_graph_view.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay, 'lr': args.decoder_lr},
        {'params': [p for n, p in model.fc_graph_view.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': args.decoder_lr},
    ]
    ...
    if args.lr_schedule == 'fixed':
        ...
        args.warmup_steps = 0.03 * max_steps
        try:
    ...
    # try:
        for epoch_id in range(args.n_epochs):
            ...
            for qids, labels, *input_data in tqdm(dataset.train()):
                ...
                if args.fp16:
                    with torch.cuda.amp.autocast():
                        loss_tuple, logits_tuple, embs_tuple = model(*input_data)
                        infonce_loss_text, infonce_loss_graph, contrastive_loss = loss_tuple
                        logits_text, logits_graph = logits_tuple
                        
                        loss = (infonce_loss_graph + infonce_loss_text) / 2
                        loss += 0.05 * contrastive_loss
                        mse_loss = nn.MSELoss()
                        loss += 0.01 * mse_loss(logits_text.view(-1), logits_graph.view(-1))
                
                else:
                    infonce_loss_text, infonce_loss_graph, contrastive_loss, logits_text, logits_graph = model(*input_data, layer_id=args.encoder_layer)
                    loss = (infonce_loss_graph + infonce_loss_text) / 2
                    loss += 0.05 * contrastive_loss
                    mse_loss = nn.MSELoss()
                    loss += 0.01 * mse_loss(logits_text.view(-1), logits_graph.view(-1))
            ...
            model.eval()
            dev_acc = evaluate_accuracy(dataset.dev(), dataset.get_corpus(), model, dataset_name=args.dataset, split='dev')
            save_test_preds = args.save_model
            # if not save_test_preds:
            test_acc = evaluate_accuracy(dataset.test(), dataset.get_corpus(), model, args.dataset, 'test') if args.test_statements else 0.0
            # else:
            ...
            #                 for i, (qid, label, pred, _preds_ranked, cids, ntype, edges, etype) in enumerate(zip(qids, targets, predictions, preds_ranked, concept_ids, node_type_ids, edge_index, edge_type)):
            ...
            #     if args.save_model:
            #         torch.save([model.state_dict(), args], f"{model_path}")
...
def eval_detail(args):
    ...
    save_test_preds = args.save_model
    dev_acc = evaluate_accuracy(dataset.dev(), dataset.get_corpus(), model, args.dataset,'dev')
