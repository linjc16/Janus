...
output_paths = {
    ...
    'fiqa': {
        'grounded': {
            'queries': 'data/fiqa/queries_grounded.jsonl',
            'corpus': 'data/fiqa/corpus_grounded.jsonl',
        },
        'graph': {
            'adj-queries': './data/fiqa/queries.graph.adj.pk',
            'adj-corpus': './data/fiqa/corpus.graph.adj.pk',
        },
    },
    'scifact': {
        'grounded': {
            'queries': 'data/scifact/queries_grounded.jsonl',
            'corpus': 'data/scifact/corpus_grounded.jsonl',
        },
        'graph': {
            'adj-queries': './data/scifact/queries.graph.adj.pk',
            'adj-corpus': './data/scifact/corpus.graph.adj.pk',
        },
    },
}
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(data):
    _id, qc_ids, ac_ids, question = data
    qa_nodes = set(qc_ids) | set(ac_ids)
    extra_nodes = set()
    for qid in qa_nodes:
        for aid in qa_nodes:
            if qid != aid and qid in cpnet_simple.nodes and aid in cpnet_simple.nodes:
                extra_nodes |= set(cpnet_simple[qid]) & set(cpnet_simple[aid])
    extra_nodes = extra_nodes - qa_nodes
    return (_id, sorted(qc_ids), sorted(ac_ids), question, sorted(extra_nodes))

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
    _id, qc_ids, ac_ids, question, extra_nodes = data
    cid2score = None
    return (_id, qc_ids, ac_ids, question, extra_nodes, cid2score)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part3(data):
    _id, qc_ids, ac_ids, question, extra_nodes, cid2score = data
    schema_graph = qc_ids + ac_ids + extra_nodes
    arange = np.arange(len(schema_graph))
    qmask = arange < len(qc_ids)
    amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids) + len(ac_ids)))
    adj, concepts = concepts2adj(schema_graph)
    return {'_id': _id,'adj': adj, 'concepts': concepts, 'qmask': qmask, 'amask': amask, 'cid2score': cid2score}
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    with open(grounded_path, 'r', encoding='utf-8') as fin_ground:
        ...
        for j, line in enumerate(lines_ground):
            ...
            if len(dic['qc']) == 0:
                # add "nothing"
                dic['qc'].append('nothing')
            q_ids = set(concept2id[c] for c in dic['qc'])
            a_ids = set(concept2id[c] for c in dic['ac'])
            q_ids = q_ids - a_ids
            QAcontext = dic['sent']
            _id = dic['id']
            qa_data.append((_id, q_ids, a_ids, QAcontext))
    ...
    # # save to output_path_{i}
    # n_parts = 10
    # n_data = len(res3)
    # n_data_per_part = n_data // n_parts
    # for i in range(n_parts + 1):
    #     output_path_i = output_path + f'_{i}'
    #     with open(output_path_i, 'wb') as fout:
    #         pickle.dump(res3[i * n_data_per_part: (i + 1) * n_data_per_part], fout)
...
if __name__ == '__main__':
    ...
    for frame in ['queries', 'corpus']:
        generate_adj_data_from_grounded_concepts__use_LM(
            output_paths[args.dataset]['grounded'][frame],
            output_paths['cpnet']['pruned-graph'],
            output_paths['cpnet']['vocab'],
            output_paths[args.dataset]['graph'][f'adj-{frame}'],
            args.nprocs,
        )