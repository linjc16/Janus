...
def ground_qa_pair(input):
    ...
    return {"id": id, "sent": s, "ans": a, "qc": question_concepts, "ac": []}
...
def match_mentioned_concepts(sents, ids, num_processes):
    res = []
    with Pool(num_processes) as p:
        res = list(tqdm(p.imap(ground_qa_pair, zip(sents, ids)), total=len(sents)))
    return res
...
def ground_retrieval(filepath, cpnet_vocab_path, pattern_path, output_path, num_processes=1):
    ...
    for line in lines:
        ...
        if 'title' in j:
            sents.append(j["title"] + " " + j["text"])
        else:
            sents.append(j["text"])
        ids.append(j["_id"])
    
    res = match_mentioned_concepts(sents, ids, num_processes)
...
if __name__ == "__main__":
    ...
    filepath = f'data/{args.dataset}'

    for frame in ['queries', 'corpus']:
        ground_retrieval(f'{filepath}/{frame}.jsonl', 'data/cpnet/concept.txt', 'data/cpnet/matcher_patterns.json', f'{filepath}/{frame}_grounded.jsonl', 32)
