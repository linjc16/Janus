...
###### Training ######
for seed in 0; do
  python3 -u src/train/qa/vis/mistral_dg_scores_dual_view.py --dataset $dataset \
      --encoder $model -k $k --gnn_dim $gnndim -elr $elr -dlr $dlr -bs $bs -mbs $mbs --fp16 true --seed $seed \
      --loss cross_entropy \
      --graph_text_embs_path data/${dataset}/graph/train.graph.embeddings_cat_query_candidate_nofilter_e5-mistral.pt \
      --graph_text_embs_path_dev data/${dataset}/graph/dev.graph.embeddings_cat_query_candidate_nofilter_e5-mistral.pt \
        --graph_text_embs_path_test data/${dataset}/graph/test.graph.embeddings_cat_query_candidate_nofilter_e5-mistral.pt \
  ...
  > logs/qa/dual_view/csqa/vis/train_dual_view_${model_prefix}_${dataset}__enc-__k${k}__gnndim${gnndim}__bs${bs}__seed${seed}__${dt}.log.txt
done