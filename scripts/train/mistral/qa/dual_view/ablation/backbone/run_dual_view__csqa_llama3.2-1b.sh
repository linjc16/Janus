...
model_prefix='decoder_llama3.2-1b'
shift
shift
args=$@
...
for seed in 0 1 2; do
  python3 -u src/train/qa/mistral_dg_scores_dual_view.py --dataset $dataset \
      ...
      --loss cross_entropy \
      --graph_text_embs_path data/csqa/graph/train.graph.embeddings_cat_query_candidate_nofilter_llama-3.2-1b.pt \
  ...
  > logs/ablation/qa/backbone/train_dual_view_${model_prefix}_${dataset}__enc-__k${k}__gnndim${gnndim}__bs${bs}__seed${seed}__${dt}.log.txt
done