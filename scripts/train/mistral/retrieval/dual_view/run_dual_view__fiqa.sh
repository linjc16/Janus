...
save_dir_pref='/data/linjc/qagnn/saved_models/retrieval/dual_view'
mkdir -p $save_dir_pref
mkdir -p logs

###### Training ######
for seed in 0 1 2; do
  python3 -u src/train/retrieval/mistral_dual_view.py --dataset $dataset \
      --encoder $model -k $k --gnn_dim $gnndim -elr $elr -dlr $dlr -bs $bs -mbs $mbs --fp16 true --seed $seed \
      --graph_text_embs_query_path data/${dataset}/graph/train.queries.graph.embeds_cat.pt \
      --graph_text_embs_corpus_path data/${dataset}/graph/train.corpus.graph.embeds_cat.pt \
  ...
  > logs/retrieval/dual_view/train_new_dual_view_${model_prefix}_${dataset}__enc-__k${k}__gnndim${gnndim}__bs${bs}__seed${seed}__${dt}.log.txt
done