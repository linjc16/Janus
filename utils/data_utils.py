...
class MultiGPUSparseAdjDataBatchGeneratorMistral(object):
    def __init__(self, args, mode, device0, device1, batch_size, indexes, qids, labels,
                 tensors0=[], lists0=[], tensors1=[], lists1=[], adj_data=None):
        self.args = args
        self.mode = mode
    ...
    def __iter__(self):
        ...
        def create_one_hot_mask(batch_labels, num_choice):
            batch_size = batch_labels.size(0)
            # one_hot_tensor = torch.zeros(batch_size, num_choice, device=batch_labels.device, dtype=torch.int64)
            # one_hot_tensor.scatter_(1, batch_labels.unsqueeze(1), 1)
            
            # # return flat_mask
            # target = torch.zeros(batch_size, batch_size * num_choice, device=batch_labels.device, dtype=torch.int64)
            
            # for i in range(batch_size):
            #     start_index = i * num_choice
            #     target[i, start_index:start_index + num_choice] = one_hot_tensor[i]
            
            target = torch.zeros(batch_size, num_choice * batch_size, device=batch_labels.device, dtype=torch.int64)
            cols = (torch.arange(batch_size, device=batch_labels.device) * num_choice + batch_labels).long()
            target.scatter_(1, cols.view(batch_size, 1), 1)
        ...
        for a in range(0, n, bs):
            ...
            ending_dict = {k: torch.stack([d['ending_dict'][k] for d in tensor0_subset], dim=0) for k in tensor0_subset[0]['ending_dict'].keys()}
            
            # for batch_ids and ending_dict, if dimention is 3, squeeze(1)
            for k in batch_dict.keys():
                if batch_dict[k].dim() == 3:
                    batch_dict[k] = batch_dict[k].squeeze(1)
            
            # for each key in dict, _to_device
            batch_dict = {k: self._to_device(v, self.device0) for k, v in batch_dict.items()}
            ending_dict = {k: self._to_device(v, self.device0) for k, v in ending_dict.items()}

            cl_target = create_one_hot_mask(batch_labels, ending_dict['input_ids'].size(1)) # contrastive learning mask
            # to_device
            cl_target = self._to_device(cl_target, self.device0)
            ...
            yield tuple([batch_qids, batch_labels, *(batch_dict, ending_dict, cl_target), *batch_lists0, *batch_tensors1, *batch_lists1, edge_index, edge_type])
...
class MultiGPUSparseAdjDataBatchGeneratorMistralRetrieval(object):
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            yield tuple([batch_qids, batch_labels, *(batch_dict, ending_dict, cl_target), *batch_lists0, *batch_tensors1_query, *batch_tensors1_corpus, *batch_lists1, edge_index, edge_type])
...
class MultiGPUSparseAdjDataBatchGeneratorMistralScores(object):
    def __init__(self, args, mode, device0, device1, batch_size, indexes, qids, labels,
                 tensors0=[], lists0=[], tensors1=[], lists1=[], adj_data=None, graph_text_embs=None):
        self.args = args
        self.mode = mode
        self.device0 = device0
        self.device1 = device1
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            yield tuple([batch_qids, batch_labels, batch_dict, *batch_lists0, *batch_tensors1, *batch_lists1, edge_index, edge_type])
...
class MultiGPUSparseAdjDataBatchGeneratorMistralScoresCatGraphsEmbs(object):
    def __init__(self, args, mode, device0, device1, batch_size, indexes, qids, labels,
                 tensors0=[], lists0=[], tensors1=[], lists1=[], adj_data=None, graph_text_embs=None):
        self.args = args
        self.mode = mode
        ...
        # self.adj_empty = adj_empty.to(self.device1)
        self.adj_data = adj_data
        self.graph_text_embs = graph_text_embs
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            if self.graph_text_embs is not None:    
                batch_graph_text_embs = self._to_device(self.graph_text_embs[batch_indexes, :, :], self.device0)
                batch_dict['graph_text_embs'] = batch_graph_text_embs
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length):
    ...
    def convert_examples_to_features_mistral(examples, label_list, max_seq_length,
                                     tokenizer):
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            query = "Given a multi-choice question and choice candidates, retrieve the correct answer. "
            # query = "Given a multi-choice question, retrieve the correct answer."
            query += example.contexts[0] + " "
            query += example.question + " " if example.question != "" else ""
            
            # for example.endings, transform to (0): endings[0], (1): endings[1], ...
            query += " ".join([f"({i}): {ending};" for i, ending in enumerate(example.endings)])
            # replace the last semicolon with a period
            query = query[:-1] + "."
            
            query = query.strip()
            
            batch_dict = tokenizer(query, max_length=max_seq_length, padding=True, truncation=True, return_tensors='pt')     
            ...
            ending_dict = tokenizer(endings, max_length=max_seq_length, padding=True, truncation=True, return_tensors='pt')
            ending_dict['input_ids'] = F.pad(ending_dict['input_ids'], (max_seq_length - ending_dict['input_ids'].shape[1], 0), value=tokenizer.pad_token_id)
            ending_dict['attention_mask'] = F.pad(ending_dict['attention_mask'], (max_seq_length - ending_dict['attention_mask'].shape[1], 0), value=0)
            ...
            features.append({
                ...
                'ending_dict': ending_dict,
                'label': label,
                'endings_text': endings,
    ...
    def convert_examples_to_features_mistral_scores(examples, label_list, max_seq_length,
                                     tokenizer):
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
                ...
                # for each ending, concatenate the query and ending
                inputs = query + " " + ending
                ...
                choices_features.append(batch_dict)
            try:
                label = label_map[example.label]
            except:
                label = example.label
    ...
    if model_type not in 'mistral':
        ...
        *data_tensors, all_label = convert_features_to_tensors(features)
        ...
        max_seq_length = 128
        # max_seq_length = 256
        features = convert_examples_to_features_mistral_scores(examples, list(range(len(examples[0].endings))), max_seq_length, tokenizer)
        example_ids = [f['example_id'] for f in features]
        all_label = torch.tensor([f['label'] for f in features], dtype=torch.long)
