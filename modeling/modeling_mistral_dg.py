...
class LM_QAGNN(nn.Module):
    ...
    def get_cosine_loss(self, anchor, positive, targets):
        ...
        def create_batch_mask(batch_size, num_choice, total_length):
            index_matrix = torch.arange(total_length).expand(batch_size, -1)
            start_indices = torch.arange(0, batch_size * num_choice, num_choice).unsqueeze(1)
            mask = (index_matrix >= start_indices) & (index_matrix < start_indices + num_choice)
            return mask
        
        num_choice = int(targets.size(1) / targets.size(0))
        mask = create_batch_mask(targets.size(0), num_choice, logits.size(1)).to(logits.device)

        # according to the mask, set value mask == 0 to -inf
        logits = logits.masked_fill(~mask, float('-inf'))

        # choose the index of the maximum value
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        batch_dict, ending_dict, cl_target = inputs[:-6]
        
        # input_ids, attention_masks in ending_dict
        for key in ending_dict:
            ending_dict[key] = ending_dict[key].view(ending_dict[key].size(0) * ending_dict[key].size(1), *ending_dict[key].size()[2:])
        ...
        choice_embs = self.encoder.module.get_input_embeddings()(ending_dict['input_ids'])
        
        kg_vecs, kg_vecs_mean, attn = self.decoder(sent_vecs_query.to(node_type_ids.device),
                                    concept_ids,
        ...
        logits_sent = output_merging_sent_embs(sent_input_embs, batch_dict, kg_vecs_mean)
        logits_ending = output_merging_sent_embs(choice_embs, ending_dict, kg_vecs.view(-1, kg_vecs.size(-1)))
        
        loss, logits, targets = self.get_cosine_loss(logits_sent, logits_ending, cl_target.float())
        
        if not detail:
            return loss, logits, targets, attn
        else:
            return loss, logits, targets, attn, concept_ids.view(bs, nc, -1), node_type_ids.view(bs, nc, -1), edge_index_orig, edge_type_orig
...
class LM_QAGNN_DataLoader(object):
    ...
    def train(self):
        ...
        return MultiGPUSparseAdjDataBatchGeneratorMistral(self.args, 'train', self.device0, self.device1, self.batch_size, train_indexes, self.train_qids, self.train_labels, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data)

    def train_eval(self):
        return MultiGPUSparseAdjDataBatchGeneratorMistral(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.train_qids)), self.train_qids, self.train_labels, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data)

    def dev(self):
        return MultiGPUSparseAdjDataBatchGeneratorMistral(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.dev_qids)), self.dev_qids, self.dev_labels, tensors0=self.dev_encoder_data, tensors1=self.dev_decoder_data, adj_data=self.dev_adj_data)

    def test(self):
        if self.is_inhouse:
            return MultiGPUSparseAdjDataBatchGeneratorMistral(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, self.inhouse_test_indexes, self.train_qids, self.train_labels, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data)
        else:
            return MultiGPUSparseAdjDataBatchGeneratorMistral(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.test_qids)), self.test_qids, self.test_labels, tensors0=self.test_encoder_data, tensors1=self.test_decoder_data, adj_data=self.test_adj_data)
