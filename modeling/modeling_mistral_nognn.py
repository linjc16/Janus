...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        loss, logits, targets = self.get_cosine_loss(sent_vecs_query, sent_vecs_choice, cl_target.float())
        
        if not detail:
            return loss, logits, targets, None
        else:
            return loss, logits, targets, None, concept_ids.view(bs, nc, -1), node_type_ids.view(bs, nc, -1), edge_index_orig, edge_type_orig
