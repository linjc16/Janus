...
class QAGNN(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
        ...
        self.fc = MLP(concept_dim + concept_dim, fc_dim, sent_dim, n_fc_layer, p_fc, layer_norm=True)
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores, adj_lengths, adj, emb_data=None, cache_output=False):
        ...
        concat_kg = self.fc(self.dropout_fc(torch.cat((graph_vecs, Z_vecs), 1)))
        concat = sent_vecs + concat_kg
        
        logits = concat
        logits = self.multi_modal_projector(logits)
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        returns: (batch_size, 1)
        """
        bs, nc = inputs[0][0].size(0), inputs[0][0].size(1)
        ...
        lm_inputs_query, lm_inputs_corpus = inputs[0][0:4], inputs[0][4:8]

        def get_kg_vec(_inputs_graph, lm_inputs, edge_index, edge_type):
            ...
            embs, attn = self.decoder(sent_vecs.to(node_type_ids.device),
                                        concept_ids,
        ...
        embs_query, _ = get_kg_vec(_inputs_graph_query, lm_inputs_query, edge_index_query, edge_type_query)
        embs_corpus, _ = get_kg_vec(_inputs_graph_corpus, lm_inputs_corpus, edge_index_corpus, edge_type_corpus)
        ...
        infonce_loss, logits_graph = output_infoNCE_loss(embs_query, embs_corpus)

        return infonce_loss, logits_graph, (embs_query, embs_corpus)
...
class LM_QAGNN_DataLoader_Retrieval(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
                 ...
                 is_inhouse=False, inhouse_train_qids_path=None, all_corpus_adj_path=None, all_corpus_statements_path=None,
    ...
    def get_corpus(self):
        return MultiGPUSparseAdjDataBatchGenerator(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.corpus_qids)), self.corpus_qids, None, tensors0=self.corpus_encoder_data, tensors1=[self.all_corpus_decoder_data] * 2, adj_data=[self.all_corpus_adj_data] * 2)
