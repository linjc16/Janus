...
class MultiGPUSparseAdjDataBatchGenerator(object):
    def __init__(self, args, mode, device0, device1, batch_size, indexes, qids, labels,
        ...
        self.qids = qids
        self.labels = labels
        self.tensors0_query, self.tensors0_corpus = tensors0
        self.lists0 = lists0
        self.tensors1_query, self.tensors1_corpus = tensors1
        self.lists1 = lists1
        # self.adj_empty = adj_empty.to(self.device1)
        self.adj_data_query, self.adj_data_corpus = adj_data
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            batch_tensors0_query = [self._to_device(x[batch_indexes], self.device0) for x in self.tensors0_query]
            batch_tensors0_corpus = [self._to_device(x[batch_indexes], self.device0) for x in self.tensors0_corpus]

            batch_tensors1_query = [self._to_device(x[batch_indexes], self.device1) for x in self.tensors1_query]
            batch_tensors1_corpus = [self._to_device(x[batch_indexes], self.device1) for x in self.tensors1_corpus]
            ...
            def get_edge_data(adj_data):
                edge_index_all, edge_type_all = adj_data
            ...
            edge_index_query, edge_type_query = get_edge_data(self.adj_data_query)
            edge_index_corpus, edge_type_corpus = get_edge_data(self.adj_data_corpus)

            
            yield tuple([batch_qids, None, (*batch_tensors0_query, *batch_tensors0_corpus), *batch_lists0, *batch_tensors1_query, *batch_tensors1_corpus, *batch_lists1, (edge_index_query, edge_index_corpus), (edge_type_query, edge_type_corpus)])
...
class MultiGPUSparseAdjDataBatchGeneratorMistralRetrieval(object):
    def __init__(self, args, mode, device0, device1, batch_size, indexes, qids, labels,
                 tensors0=[], lists0=[], tensors1=[], lists1=[], adj_data=None,
                 graph_text_embs_query=None, graph_text_embs_corpus=None):
        self.args = args
        self.mode = mode
        self.device0 = device0
        self.device1 = device1
        ...
        # self.adj_empty = adj_empty.to(self.device1)
        self.adj_data_query, self.adj_data_corpus = adj_data
        self.graph_text_embs_query = graph_text_embs_query
        self.graph_text_embs_corpus = graph_text_embs_corpus
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            # for self.tensor0, extract batch_dict and ending_dict
            tensor0_subset = [self.tensors0[idx] for idx in batch_indexes]
            # merge the tensor with the same key for each element in the list
            
            # stack tensor0_subset[i]['choices_features'][j]
            batch_dict_choices = {
                key: torch.stack([choice_dict[key] for item in tensor0_subset for choice_dict in item['choices_features']], dim=0)
                for key in tensor0_subset[0]['choices_features'][0].keys()
            }
            
            batch_dict_queries = {
                key: torch.stack([query_dict[key] for item in tensor0_subset for query_dict in item['queries_features']], dim=0)
                for key in tensor0_subset[0]['queries_features'][0].keys()
            }
            
            # batch_dict = {k: torch.stack([d['batch_dict'][k] for d in tensor0_subset], dim=0) for k in tensor0_subset[0]['batch_dict'].keys()}
            
            # for batch_ids and ending_dict, if dimention is 3, squeeze(1)
            for k in batch_dict_choices.keys():
                if batch_dict_choices[k].dim() == 3:
                    batch_dict_choices[k] = batch_dict_choices[k].squeeze(1)
            
            for k in batch_dict_queries.keys():
                if batch_dict_queries[k].dim() == 3:
                    batch_dict_queries[k] = batch_dict_queries[k].squeeze(1)
            
            # for each key in dict, _to_device
            batch_dict_choices = {k: self._to_device(v, self.device0) for k, v in batch_dict_choices.items()}
            batch_dict_queries = {k: self._to_device(v, self.device0) for k, v in batch_dict_queries.items()}
            ...
            batch_dict_choices['graph_text_embs'] = None
            batch_dict_queries['graph_text_embs'] = None
            
            if self.graph_text_embs_corpus is not None:    
                batch_graph_text_embs_corpus = self._to_device(self.graph_text_embs_corpus[batch_indexes, :, :], self.device0)
                batch_dict_choices['graph_text_embs'] = batch_graph_text_embs_corpus
            
            if self.graph_text_embs_query is not None:
                batch_graph_text_embs_query = self._to_device(self.graph_text_embs_query[batch_indexes, :, :], self.device0)
                batch_dict_queries['graph_text_embs'] = batch_graph_text_embs_query
            
            yield tuple([batch_qids, None, (batch_dict_queries, batch_dict_choices), *batch_lists0, *batch_tensors1_query, *batch_tensors1_corpus, *batch_lists1, (edge_index_query, edge_index_corpus), (edge_type_query, edge_type_corpus)])
...
def load_sparse_adj_data_with_contextnode_retrieval(adj_pk_path, max_node_num, num_choice, frame, args):
    cache_path = adj_pk_path + f'_{frame}.loaded_cache'
    use_cache = True
    ...
    if use_cache:
        ...
        for idx, _data in tqdm(enumerate(adj_concept_pairs), total=n_samples, desc='loading adj matrices'):
            if frame == 'query' or frame == 'corpus':
                _data_query, _data_corpus = _data[0], _data[1]
            ...
            if frame == 'query':
                ...
                raise ValueError('frame should be either "query" or "corpus"')
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length):
    ...
    class InputFeatures(object):
        ...
        def __init__(self, example_id, choices_features, queries_features, label):
            self.example_id = example_id
            ...
            self.queries_features = [
                ...
                for _, input_ids, input_mask, segment_ids, output_mask in queries_features
            ]
            self.label = label
    ...
    def convert_examples_to_features_retrieval(examples, label_list, max_seq_length,
                                     tokenizer,
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
                ...
                def _get_batch_dict(input_text):
                    ...
                    return tokens, input_ids, input_mask, segment_ids, output_mask

                tokens, input_ids, input_mask, segment_ids, output_mask = _get_batch_dict(ending)
                choices_features.append((tokens, input_ids, input_mask, segment_ids, output_mask))

                tokens_query, input_ids_query, input_mask_query, segment_ids_query, output_mask_query = _get_batch_dict(context)
                queries_features.append((tokens_query, input_ids_query, input_mask_query, segment_ids_query, output_mask_query))
            try:
                label = label_map[example.label]
            except:
                label = example.label
            features.append(InputFeatures(example_id=example.example_id, choices_features=choices_features, queries_features=queries_features, label=label))

        return features


    def convert_examples_to_features_mistral_retrieval(examples, label_list, max_seq_length,
                                     tokenizer):
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            for ending_idx, (query, ending) in enumerate(zip(example.contexts, example.endings)):
                
                def _get_batch_dict(inputs):
                    batch_dict = tokenizer(inputs, max_length=max_seq_length, padding=True, truncation=True, return_tensors='pt')     
                    # # add -1 at the begginging of batch_dict['input_ids'], representing the graph token
                    # batch_dict['input_ids'] = F.pad(batch_dict['input_ids'], (1, 0), value=-1)
                    # batch_dict['attention_mask'] = F.pad(batch_dict['attention_mask'], (1, 0), value=1)
                    
                    # pad the input_ids and attention_mask in batch_dict to max_seq_length
                    # pad_on_left
                    batch_dict['input_ids'] = F.pad(batch_dict['input_ids'], (max_seq_length - batch_dict['input_ids'].shape[1], 0), value=tokenizer.pad_token_id)
                    batch_dict['attention_mask'] = F.pad(batch_dict['attention_mask'], (max_seq_length - batch_dict['attention_mask'].shape[1], 0), value=0)
                    
                    if "token_type_ids" in batch_dict:
                        batch_dict['token_type_ids'] = F.pad(batch_dict['token_type_ids'], (max_seq_length - batch_dict['token_type_ids'].shape[1], 0), value=0)

                    return batch_dict
                
                if 'scifact' in statement_jsonl_path:
                    query = 'Given a scientific claim, retrieve documents that support or refute the claim.\n' + query
                elif 'fiqa' in statement_jsonl_path:
                    query = 'Given a financial question, retrieve user replies that best answer the question.\n' + query
                else:
                    raise ValueError('statement_jsonl_path should be either scifact or fiqa')
                
                query_dict = _get_batch_dict(query)
                ending_dict = _get_batch_dict(ending)

                choices_features.append(ending_dict)
                queries_features.append(query_dict)
            try:
                label = label_map[example.label]
            except:
                label = example.label
    ...
    def select_field(features, field, frame='query'):
        if frame == 'query':
            return [[choice[field] for choice in feature.queries_features] for feature in features]
        elif frame == 'corpus':
            return [[choice[field] for choice in feature.choices_features] for feature in features]

    def convert_features_to_tensors(features, frame='query'):
        all_input_ids = torch.tensor(select_field(features, 'input_ids', frame), dtype=torch.long)
        all_input_mask = torch.tensor(select_field(features, 'input_mask', frame), dtype=torch.long)
        all_segment_ids = torch.tensor(select_field(features, 'segment_ids', frame), dtype=torch.long)
        all_output_mask = torch.tensor(select_field(features, 'output_mask', frame), dtype=torch.bool)
    ...
    if model_type not in 'mistral':
        features = convert_examples_to_features_retrieval(examples, list(range(len(examples[0].endings))), max_seq_length, tokenizer,
        ...
        *data_tensors_corpus, all_label = convert_features_to_tensors(features, frame='corpus')
        *data_tensors_query, _ = convert_features_to_tensors(features, frame='query')
        
        return (example_ids, all_label, (data_tensors_query, data_tensors_corpus))
    
    else:
        # max_seq_length = 128
        max_seq_length = 256
        features = convert_examples_to_features_mistral_retrieval(examples, list(range(len(examples[0].endings))), max_seq_length, tokenizer)
        example_ids = [f['example_id'] for f in features]
        all_label = torch.tensor([f['label'] for f in features], dtype=torch.long)
        
        return (example_ids, all_label, features)
        


def load_input_tensors_retrieval(input_jsonl_path, model_type, model_name, max_seq_length):
    if model_type in ('lstm',):
        raise NotImplementedError
    elif model_type in ('gpt',):
        return load_gpt_input_tensors(input_jsonl_path, max_seq_length)
    elif model_type in ('bert', 'xlnet', 'roberta', 'albert', 'mistral'):
