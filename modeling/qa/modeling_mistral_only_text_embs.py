...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        graph_text_embs = self.fc_view2(graph_text_embs)
        ...
        logits_sent = output_merging_sent_embs(sent_input_embs, batch_dict, graph_text_embs.view(-1, graph_text_embs.size(2)))
        
        
        logits = self.fc(logits_sent).view(bs, nc)
        
        if not detail:
            return logits, None
        else:
            return logits, None, concept_ids.view(bs, nc, -1), node_type_ids.view(bs, nc, -1), edge_index_orig, edge_type_orig
