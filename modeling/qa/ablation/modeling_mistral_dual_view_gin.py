...
class QAGNN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,
        ...
        self.k = k
        self.gnn_layers = nn.ModuleList([GINConvE(args, hidden_size, n_ntype, n_etype, self.edge_encoder) for _ in range(k)])
...
class GINConvE(MessagePassing):
    def __init__(self, args, emb_dim, n_ntype, n_etype, edge_encoder, aggr="mean"):
        super(GINConvE, self).__init__(aggr=aggr)
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra):
        ...
        out = (1 + self.eps) * self.linear(x) + aggr_out
