...
class QAGNN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,
        ...
        self.k = k
        self.gnn_layers = nn.ModuleList([GPRGNNConvE(args, hidden_size, n_ntype, n_etype, self.edge_encoder) for _ in range(k)])
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        return logits_text, logits_graph, contrastive_loss
...
class GPRGNNConvE(MessagePassing):
    def __init__(self, args, emb_dim, n_ntype, n_etype, edge_encoder, aggr="mean"):
        super(GPRGNNConvE, self).__init__(aggr=aggr)
        ...
        # MLP for post-processing the aggregated messages
        ...
        self.linear_msg = nn.Linear(3 * emb_dim, emb_dim)
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra):
        ...
        edge_index, norm = gcn_norm(
            edge_index, None, num_nodes=x.size(0), dtype=x.dtype)

        x = torch.cat([x, node_feature_extra], dim=1)

        hidden = self.linear(x) * (self.temp[0])
        for k in range(self.K):
            aggr_out = self.propagate(edge_index, x=x, edge_attr=edge_embeddings, norm=norm) #[N, emb_dim]
            gamma = self.temp[k+1]
            hidden = hidden + gamma * aggr_out

        out = self.mlp(hidden)
        return out
    
    def message(self, x_j, edge_attr, norm):
        # Message function simply sums the neighboring node features and edge attributes, if available
        out = self.linear_msg(torch.cat([x_j, edge_attr], dim=1))
