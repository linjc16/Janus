...
from modeling.qa.vis.data_utils import *
...
class LM_QAGNN(nn.Module):
    def __init__(self, args, model_name, k, n_ntype, n_etype,
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.encoder.sent_dim, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 1)
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, split='train'):
        """
        ...
        _inputs_graph = [x.view(x.size(0) * x.size(1), *x.size()[2:]) for x in inputs[-6:-2]] + [sum(x,[]) for x in inputs[-2:]]
        
        concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs_graph
        ...
        # logits_sent_text = output_merging_sent_embs(sent_input_embs, batch_dict, graph_text_embs.view(-1, graph_text_embs.size(2)))
        logits_sent_text = graph_text_embs.view(-1, graph_text_embs.size(2))
        logits_text = self.fc(logits_sent_text).view(bs, nc)
        
        logits_sent_graph = output_merging_sent_embs(sent_input_embs, batch_dict, kg_vecs.squeeze(1))
        logits_sent_graph = self.fc_graph_view(logits_sent_graph)
        logits_graph = self.fc(logits_sent_graph).view(bs, nc)
        
        contrastive_loss = self.get_cosine_loss(logits_sent_text, logits_sent_graph)
        
        return logits_text, logits_graph, contrastive_loss, (logits_sent_text, logits_sent_graph)
...
class LM_QAGNN_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
                 ...
                 is_inhouse=False, inhouse_train_qids_path=None, graph_text_embs_path=None,
                 graph_text_embs_path_dev=None, graph_text_embs_path_test=None,
        ...
        *self.dev_decoder_data, self.dev_adj_data = load_sparse_adj_data_with_contextnode(dev_adj_path, max_node_num, num_choice, args)
        ...
        if subsample < 1.:
            ...
            if self.is_inhouse:
                ...
                self.train_encoder_data = self.train_encoder_data[:n_train]
                self.train_decoder_data = [x[:n_train] for x in self.train_decoder_data]
                self.train_adj_data = self.train_adj_data[:n_train]
                # assert all(len(self.train_qids) == len(self.train_adj_data[0]) == x.size(0) for x in [self.train_labels] + self.train_encoder_data + self.train_decoder_data)
            assert self.train_size() == n_train
        
        self.graph_text_embs = None
        if graph_text_embs_path is not None:
            self.graph_text_embs = torch.load(graph_text_embs_path)
            self.graph_text_embs = self.graph_text_embs.view(-1, num_choice, self.graph_text_embs.size(1))
        
        self.graph_text_embs_dev = None
        if graph_text_embs_path_dev is not None:
            self.graph_text_embs_dev = torch.load(graph_text_embs_path_dev)
            self.graph_text_embs_dev = self.graph_text_embs_dev.view(-1, num_choice, self.graph_text_embs_dev.size(1))

        self.graph_text_embs_test = None
        if graph_text_embs_path_test is not None:
            self.graph_text_embs_test = torch.load(graph_text_embs_path_test)
            self.graph_text_embs_test = self.graph_text_embs_test.view(-1, num_choice, self.graph_text_embs_test.size(1))
    ...
    def train(self):
        ...
        return MultiGPUSparseAdjDataBatchGeneratorMistralScoresCatGraphsEmbs(self.args, 'train', self.device0, self.device1, self.batch_size, train_indexes, self.train_qids, self.train_labels, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data, graph_text_embs=self.graph_text_embs)
    
    def train_eval(self):
        return MultiGPUSparseAdjDataBatchGeneratorMistralScoresCatGraphsEmbs(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.train_qids)), self.train_qids, self.train_labels, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data, graph_text_embs=self.graph_text_embs)

    def dev(self):
        return MultiGPUSparseAdjDataBatchGeneratorMistralScoresCatGraphsEmbs(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.dev_qids)), self.dev_qids, self.dev_labels, tensors0=self.dev_encoder_data, tensors1=self.dev_decoder_data, adj_data=self.dev_adj_data, graph_text_embs=self.graph_text_embs_dev)
    
    def test(self):
        if self.is_inhouse:
            return MultiGPUSparseAdjDataBatchGeneratorMistralScoresCatGraphsEmbs(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, self.inhouse_test_indexes, self.train_qids, self.train_labels, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data, graph_text_embs= self.graph_text_embs)
        else:
            return MultiGPUSparseAdjDataBatchGeneratorMistralScoresCatGraphsEmbs(self.args, 'eval', self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.test_qids)), self.test_qids, self.test_labels, tensors0=self.test_encoder_data, tensors1=self.test_decoder_data, adj_data=self.test_adj_data, graph_text_embs= self.graph_text_embs_test)
